...
- Displaying performance results on the website:
  ...
  3.  The generated report is placed in the `benchmarks/report/` directory, and is thus
...
3. Generate HTML report
   ...
   - `publish` generates the HTML report and stores it in `benchmarks/report/`, as configured in `asv.conf.json`.
   - `preview` starts a local server and opens the report in the browser.

   NOTE: Since the results are stored in `benchmarks/report/`, you can also view the results
   with `mkdocs serve` and navigating to `http://localhost:9000/django-components/benchmarks/`.

   NOTE 2: Running `publish` will overwrite the existing contents of `benchmarks/report/`.
