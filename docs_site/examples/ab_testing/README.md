...
Use the [`Component.on_render()`][Component.on_render] hook, to decide which version to render based on a component parameter (or a random choice).

![A/B Testing](./images/ab_testing.png)

## How it works

[`Component.on_render()`][Component.on_render] is called when the component is being rendered. This method can completely override the rendering process, so we can use it to render another component in its place.
...
```djc_py
--8<-- "docs_site/examples/ab_testing/component.py"
```
...
```djc_py
--8<-- "docs_site/examples/ab_testing/page.py"
```
