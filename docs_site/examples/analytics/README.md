...
Use the [`Component.on_render_after()`][Component.on_render_after] hook to track component analytics, such as capturing errors for a service like Sentry or other monitoring.

![Analytics example](./images/analytics.png)

## Error tracking components

You can create a wrapper component that uses the [`Component.on_render_after()`][Component.on_render_after] hook to inspect the `error` object. If an error occurred during the rendering of its children, you can capture and send it to your monitoring service.
...
To do that, we can use the [`on_component_rendered()`][ComponentExtension.on_component_rendered] hook to capture all errors.
...
```djc_py
--8<-- "docs_site/examples/analytics/component.py"
```
...
```djc_py
--8<-- "docs_site/examples/analytics/page.py"
```
