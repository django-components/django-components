...
def copy_passthroughs(output_dir: Path, passthroughs: list[tuple[Path, str]]) -> list[tuple[str, int]]:
    """
    Copy pre-built static directories verbatim into the built site at their mount
    path, skipping the markdown pipeline (the content is already HTML).

    Used for the asv benchmark report (`benchmarks/report/` -> `/benchmarks/`),
    which is generated by `asv publish` and only needs to be served as-is. Each
    passthrough is a ``(source_dir, mount_path)`` pair; ``mount_path`` is relative
    to the site root. Like ``collect_static``, this belongs to the deploy / root
    assembly step (not per-version snapshots), so the report is mounted once at
    the root rather than duplicated into every version dir.

    A missing source dir is skipped (the report may not be built yet on a fresh
    clone). Returns a list of ``(mount_path, files_copied)`` for reporting.
    """
    copied: list[tuple[str, int]] = []
    for source_dir, mount_path in passthroughs:
        if not source_dir.is_dir():
            continue
        dest_root = output_dir / mount_path
        n = 0
        for src in source_dir.rglob("*"):
            if src.is_dir():
                continue
            dest = dest_root / src.relative_to(source_dir)
            dest.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(src, dest)
            n += 1
        copied.append((mount_path, n))
    return copied
