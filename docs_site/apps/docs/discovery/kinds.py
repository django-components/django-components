...
# The renderer to dispatch an entry to. These map to the per-kind components
# under ``apps/docs/components/reference/entries/`` (spike 5 section 3.2). Several
...
class ReferenceEntry(NamedTuple):
    ...
    Attributes:
        ...
        dotted_path: The public, top-level import path, e.g.
            ...
            ``{% docstring %}`` tag resolves and what the legacy anchor is built
            from - NOT griffe's canonical path
