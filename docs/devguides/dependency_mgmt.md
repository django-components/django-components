...
1. First of all, when we consider a component, it has two kind of dependencies - the "inlined" JS and CSS (from `Component.js` / `Component.css`), and additional JS and CSS via `Media.js/css` (paths, URLs, or [`Script`][Script] / [`Style`][Style] objects):
