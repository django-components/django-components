...
class BaseNode(Node, metaclass=NodeMeta):
    ...
    1. It declares how a particular template tag should be parsed - By setting the
       [`tag`][BaseNode.tag],
       [`end_tag`][BaseNode.end_tag],
       and [`allowed_flags`][BaseNode.allowed_flags] attributes:
    ...
    2. The [`render`][BaseNode.render] method is
    ...
    [`render`][BaseNode.render] method's signature.

    For more info, see [`BaseNode.render()`][BaseNode.render].
    """
    ...
    flags: dict[str, bool]
    """
    Dictionary of all [`allowed_flags`][BaseNode.allowed_flags]
    ...
    Unlike [`contents`][BaseNode.contents],
    ...
    """
    If the template that contains this node belongs to a [`Component`][Component],
    then this will be the [`Component`][Component] class.
    """
    ...
    def parse(cls, parser: Parser, token: Token, **kwargs: Any) -> "BaseNode":
        ...
        To register the tag, you can use [`BaseNode.register()`][BaseNode.register].
        """
...
def template_tag(
    ...
    """
    A simplified version of creating a template tag based on [`BaseNode`][BaseNode].

    Instead of defining the whole class, you can just define the
    [`render()`][BaseNode.render] method.
    ...
    The given function will be wrapped in a class that inherits from [`BaseNode`][BaseNode].
    ...
    - `node` is the [`BaseNode`][BaseNode] instance.
